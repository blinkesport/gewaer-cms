<template>
    <label :class="classObject" class="component-switch">

        <span class="vue-switcher__label">
            <span v-if="label" v-text="label"/>
            <span v-if="!label && enabled" class="toggle-label" v-text="textEnabled"/>
            <span v-if="!label && !enabled" class="toggle-label" v-text="textDisabled"/>
        </span>
        ...
        <div v-if="enabled" class="enabled">
            <slot name="onLabel"><span/></slot>
        </div>
        <div v-else class="disabled">
            <slot name="offLabel"><span/></slot>
        </div>
        <span v-if="sideLabel" v-text="sideLabel"/>
    </label>
</template>
...
export default {
    ...
    computed: {
        classObject() {
            return {
                "vue-switcher": true,
                "vue-switcher--unchecked": !this.enabled,
                "vue-switcher--disabled": this.disabled,
                "vue-switcher--bold": this.typeBold,
                "vue-switcher--bold--unchecked": this.typeBold && !this.enabled,
    ...
    methods: {
        change() {
            this.$emit("change", this.enabled ? 1 : 0);
        }
    }
};
</script>

<!--  Non Scoped styles mainly for scoped elements used -->
<style>
.component-switch .disabled span,
    .component-switch .enabled span {
    ...
    /* Used for enabled/disabled prop */
    .vue-switcher__label .toggle-label {
    display: block;
    font-size: 14px;
    margin-bottom: 0px;
    float: right;
